...
func TestMultipleResults(t *testing.T) {
	...
	if err := db.QueryRow("NEXT").Scan(&val); err != nil {
		t.Fatal(err)
	}
	if val != 2 {
		t.Fatalf("expected 2, but found %d", val)
	}
	if err := db.QueryRow("NEXT").Scan(&val); err != ErrNoMoreResults {
...
func TestTxnMultipleResults(t *testing.T) {
	...
	if err := tx.QueryRow("NEXT").Scan(&val); err != nil {
		t.Fatal(err)
	}
	if val != 2 {
		t.Fatalf("expected 2, but found %d", val)
	}
	if err := tx.QueryRow("NEXT").Scan(&val); err != ErrNoMoreResults {
		t.Fatalf("expected %s, but found %v", ErrNoMoreResults, err)
	}
}
